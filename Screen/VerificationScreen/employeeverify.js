...
export default function Employeeverify({navigation}) {
  const employeeotp=()=>{
    navigation.navigate('employeeotp')
  }


  const employeechangeemail=()=>{
    navigation.navigate('employeechangeemail')
  }
  return (
 ...
 <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? "padding" : "height"}  style={{marginTop:20,marginHorizontal:windowWidth/20,justifyContent:"center"}}>
   <Button onPress={employeeotp} title="Send Verification Code"/>
   <Button2 onPress={employeechangeemail} title="Change Email Id"/>
